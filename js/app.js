...
// ===== Game Selection =====
let selectedGame = "league_of_legends";
let availableGames = [];
...
function loadGames() {
    fetch("/api/games")
        ...
        .catch(() => {
            availableGames = [
                { id: "league_of_legends", name: "League of Legends", description: "MOBA teamfights & objectives" },
...
function getGameTag(game) {
    ...
    if (id === "league_of_legends") return { label: "Recommended", cls: "recommended" };
    return null;
}
...
let selectedLibraryVods = new Set();
let currentClips = [];
...
function loadLibrary() {
    ...
    <label
        ...
        <input
            ...
            value="${escapeHtml(vod.filename)}"

            ${selectedLibraryVods.has(vod.filename) ? "checked" : ""}

            onchange="toggleLibrarySelection(this)">
...
function toggleLibrarySelection(cb) {

    if (cb.checked) {

        selectedLibraryVods.add(cb.value);

    } else {

        selectedLibraryVods.delete(cb.value);

    }

    document.getElementById("queue-selected-count").textContent =
        `Selected: ${selectedLibraryVods.size}`;

    document.getElementById("btn-analyze-selected").disabled =
        selectedLibraryVods.size === 0;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const selectAll =
        document.getElementById("queue-select-all");

    if (!selectAll) return;

    selectAll.addEventListener("change", function () {

        document.querySelectorAll(".library-checkbox")
            .forEach(cb => {
...
function queueSelectedVods() {

    if (selectedLibraryVods.size === 0)
        return;
    ...
    fetch("/api/queue/add", {
        ...
        body: JSON.stringify({
            ...
            files: [...selectedLibraryVods],

            api_key: document.getElementById("api-key").value.trim(),

            game: selectedGame,

            detection_method: document.getElementById("detection-method").value,
    ...
    .then(data => {
        ...
        showNotification(
            `${selectedLibraryVods.size} VOD(s) added to queue.`
        );

selectedLibraryVods.clear();
...
document.getElementById("btn-analyze-selected").disabled = true;
...
function analyzeFromLibrary(filename) {
    ...
    // Preserve the same analysis settings used by URL and upload analysis.
    ...
    fetch("/api/analyze", {
        ...
        body: JSON.stringify({
            ...
            detection_overrides: detectionOverrides
        }),
    })
    .then(res => res.json())
    .then(data => {
...
async function loadDatasetStats() {
    ...
    document.getElementById("stat-labeled").textContent =
        stats.labeled;

    document.getElementById("stat-unlabeled").textContent =
        stats.unlabeled;

    document.getElementById("stat-highlight").textContent =
        stats.highlights;
...
<div class="clip-actions">
    ...
    <button
        class="btn-download"
        onclick="event.stopPropagation(); downloadClip('${clip.id}', '${clip.filename}')">
        Download
    </button>

    <button
        class="btn-trim"
        onclick="event.stopPropagation(); previewClip(${i})">
        Trim
    </button>

    <button
        class="btn-label"
        onclick="event.stopPropagation(); openAnnotation(currentJobId, '${clip.id}')">
        Label
    </button>

    <button
        class="btn-delete"
        onclick="event.stopPropagation(); deleteClip('${clip.id}', ${i})">
        Remove
    </button>
